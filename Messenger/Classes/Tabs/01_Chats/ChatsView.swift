...
class ChatsView: UIViewController {
	...
	func loadChats() {
		...
		dbchats = realm.objects(DBChat.self).filter(predicate).sorted(byKeyPath: FCHAT_LASTMESSAGEDATE, ascending: false)
...
extension ChatsView: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		let dbchat = dbchats[indexPath.row]
