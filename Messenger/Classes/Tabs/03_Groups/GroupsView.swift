...
class GroupsView: UIViewController {
	...
	private var dbgroups = realm.objects(DBGroup.self).filter(falsepredicate)
	...
	@objc func loadGroups() {
		...
		dbgroups = realm.objects(DBGroup.self).filter(predicate).sorted(byKeyPath: FGROUP_NAME)
