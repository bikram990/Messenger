...
class SelectUserView: UIViewController {
	...
	func loadBlockers() {
		...
		let dbblockers = realm.objects(DBBlocker.self).filter(predicate)

		for dbblocker in dbblockers {
	...
	func loadFriends() {
		...
		let predicate = NSPredicate(format: "isDeleted == NO")
		let dbfriends = realm.objects(DBFriend.self).filter(predicate)
	...
	func setObjects() {
		...
		if let sorted = collation.sortedArray(from: Array(dbusers), collationStringSelector: selector) as? [DBUser] {
