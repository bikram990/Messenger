...
class PushNotification: NSObject {
	...
	class func send(message: FObject) {
		...
		if let dbchat = realm.object(ofType: DBChat.self, forPrimaryKey: chatId) {
	...
	class func send(userIds: [String], text: String) {
		...
		let dbusers = realm.objects(DBUser.self).filter(predicate).sorted(byKeyPath: FUSER_FULLNAME)
