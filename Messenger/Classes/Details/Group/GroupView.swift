...
class GroupView: UIViewController {
	...
	@objc func loadGroup() {
		...
		dbgroup = realm.object(ofType: DBGroup.self, forPrimaryKey: groupId)

		labelName.text = dbgroup.name

		if let dbuser = realm.object(ofType: DBUser.self, forPrimaryKey: dbgroup.userId) {
