...
class AllMediaView: UIViewController {
	...
	func loadMedia() {
		...
		let dbmessages = realm.objects(DBMessage.self).filter(predicate).sorted(byKeyPath: FMESSAGE_CREATEDAT)
	...
	func presentPicture(dbmessage: DBMessage) {
		...
		if (DownloadManager.pathPhoto(dbmessage.objectId) != nil) {
			let result = PictureView.photos(messageId: dbmessage.objectId, chatId: chatId)
