...
class ChatPrivateView: RCMessagesView, UIGestureRecognizerDelegate {
	...
	private var dbchats = realm.objects(DBChat.self).filter(falsepredicate)
	...
	func loadChats() {
		...
		dbchats = realm.objects(DBChat.self).filter(predicate)
	}
	...
	func updateTitleDetails() {
		...
		if let dbuser = realm.object(ofType: DBUser.self, forPrimaryKey: recipientId) {
			labelTitle1.text = dbuser.fullname
			labelTitle2.text = Userx.lastActive(dbuser: dbuser)
		}
	}
